...
if($rowMinus=mysqli_fetch_array($rsStockMinus)){
    ...
    if( $currentStock < $quantity)
        {
            $url = "Location:admin-outward-add.php?order_id=$order_id&outofstock=yes";
            header($url);
            die(" out of stock");
        }
...
$qstockPlus="select * from stocks where  user_id = $dealerUserId and product_id = $product_id ";
...
print $query;
header("Location:admin-outward-view.php");
